...
relatorio_path = os.path.join(output_path, "relatorios", "classificao_cnn")
...
# Modelo compatível com o treinamento
...
# Função principal de classificação por varredura
def gerar_imagem_classificada_com_legenda_varredura(model, nome_base, bloco_linhas=100):
    h, w = SCL.shape
    resultado = np.full((h, w), 255, dtype=np.uint8)
    total_blocos = (h + bloco_linhas - 1) // bloco_linhas
    todas_preds = []

    print(f"[INFO] Processando {total_blocos} blocos...")
    ...
    with torch.no_grad():
        for idx, i in enumerate(range(0, h, bloco_linhas), start=1):
            ...
            # Redimensionar para (N, 3, 1, 1)
            n = X_bloco.shape[0]
            X_tensor = torch.tensor(X_bloco.reshape(n, 3, 1, 1), dtype=torch.float32).to(DEVICE)

            y_pred = model(X_tensor)
            y_pred_labels = torch.argmax(y_pred, dim=1).cpu().numpy()
            todas_preds.extend(y_pred_labels)

            rows, cols = np.where(bloco_mask)
            resultado[i + rows, cols] = y_pred_labels

            print(f"[INFO] Bloco {idx}/{total_blocos} ({i}–{i_fim}) classificado.")
    ...
    with open(os.path.join(relatorio_path, "resumo.txt"), "w", encoding="utf-8") as f:
        f.write("==== RELATÓRIO DE CLASSIFICAÇÃO - CNN ====\n\n")
    ...
    print(f"[INFO] Imagens e relatórios salvos com sucesso.")
...
model.load_state_dict(torch.load(os.path.join(modelo_path, "cnn_torch.pt"), map_location=DEVICE))

# Classificar
gerar_imagem_classificada_com_legenda_varredura(model, "classificado_cnn")
